...
Seq_ID_NEW_TYPES <- list.files(all.files = FALSE)


file.copy(Seq_ID_NEW_TYPES, "../TMP_TYPES")

setwd("../TMP_TYPES")

dir_combined_BACK_NEW <- getwd()

setwd("..")
setwd("../EUKARYOTYPING/REFERENCE_POPULATION")

Seq_ID_BACKGROUND <- list.files(all.files = FALSE)
file.copy(Seq_ID_BACKGROUND, dir_combined_BACK_NEW)

setwd(dir_combined_BACK_NEW)

Seq_ID <- list.files(all.files = FALSE) #this is all Seq_IDs - background and new.
