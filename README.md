...
This repository contains the **Alpha test version** of the workflow developed by Dr Joel Barratt at the Centers for Disease Control and Prevention (CDC) for genotyping _Cyclospora cayetanensis_. This version of the workflow has been released for academic and testing purposes. For this reason many inputs and outputs are written to temporary (TMP) directories at various stages to facilitate examination of outputs during testing. This version of the workflow is quite stable and should function reliably though may be slow and inefficient partly due to the writing of many intermediate steps of the workflow to temporary files (for testing purposes). Again, this is **a test version only** that has not been optimized for speed or for efficiency (in terms of computational resource use), though represents a relatively stable (albeit early) alpha release of CDCs _Cyclospora cayetanensis_ genotyping workflow.

**This workflow forms the basis of a US-wide _Cyclospora cayetanensis_ genotyping system developed at CDC to facilitate detection of cyclosporiasis outbreaks. Agencies wishing to partner with the CDC as part of the _Cyclospora cayetanensis_ genotyping network should contact the CDC at CyclosporaAMD@cdc.gov for the latest updates to this software and instructions on how to participate.**

The workflow comprised of three modules that perform three major tasks that facilitate identification of genetically related clusters of _Cyclospora cayetanensis_ infections:
...
This code was developed and tested using a Mac running OSX Catalina 10.15.3 and Mac OSX High Sierra 10.13.6. Further testing has verified this code on a Mac runnning OSX Big Sur 11.2. Subsequent instructions are provided _only_ for installing it on an OSX system.
...
Go to [CRAN](https://cran.r-project.org/bin/macosx/), download and install `R-4.0.4 or later`
...
-D /absolute/path/to/your/TEST_DATA \
...
```
>This will execute a series of R scripts that calculate genetic distances using the Barratt-Plucinski ("Eukaryotyping") ensemble using the haplotype data sheet printed by Module 1 (found in the haplotype_sheets directory) as input. Percent progress will be printed to the terminal window as a decimal for each of the two algorithms (Plucinski's Bayesian algorithm and Barratt's Heuristic algorithm) separately. Note that Module 2 may take a while to run to completion (it can be **really slow**) depending on the number of threads you dedicate to the computation and the number of specimens being run. Module 2 will generate a new ensemble matrix which will be printed to the ensemble_matrices directory. For the impatient among you an example ensemble matrix is provided in the ensemble_matrices directory that can be used as the input for Module 3 simply by executing the shell script as described below.
...
```
>This will execute scripts that determine an appropriate number of clusters from a heirarchical tree and print the cluster memberships to the clusters_detected directory. The ensemble matrix generated by Module 2 (or the example matrix provided) can be used as input. Using the example matrix, Module 3 should predict 21 clusters assuming the settings in the Module 3 shell script are not modified by the user.
